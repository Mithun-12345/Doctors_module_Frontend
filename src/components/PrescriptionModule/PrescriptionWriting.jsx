...
const [selectedModalContext, setSelectedModalContext] = useState({ itemId: null, medicineId: null });
const [currentItemId, setCurrentItemId] = useState(null); // To track which item we're editing
...
const removeMedicineFromMixedGroup = (mixedItemId, medicineId) => {
  ...
  onClick={() => navigate("/doctor-dashboard/all")}
    className=" mr-5
     flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-medium px-5 py-2 rounded-lg shadow-sm transition"
  >
    Back to Dashboard
    <span className="text-xl">›</span>
      ...
      <div className="pb-12 grid grid-cols-1 lg:grid-cols-2 gap-16 mb-6 mt-8 pl-[20px] pr-[20px] ">
      {/* Patient Info - New Design */}
      <div className="bg-white rounded-xl shadow-lg border-2 border-blue-200 p-5">
  {/* Main container changed to grid with 2 rows */}
  <div className="grid grid-rows-2 gap-6">

    {/* Row 1: Avatar + Name & Phone */}
    <div className="flex  gap-6 items-center"> {/* Changed items-center to items-start for better vertical alignment */}
      {/* Avatar */}
    ...
    {/* Row 2: Patient Details Grid (Full Width) */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 gap-x-10 text-sm">
      ...
      <div className="space-y-1 pl-2 ">
        ...
        <p className="pb-2">
          ...
          <span className="text-gray-700">
            {patientHistory?.patientDetails?.gender ||
              patientData?.gender ||
              "N/A"}
          </span>
        </p>
      ...
      <div className="space-y-1">
        ...
        <p className="pb-2">
          ...
          <span className="text-gray-700">
            {patientHistory?.patientDetails?.address || "-"},{" "}
            {patientHistory?.patientDetails?.city || ""}
          </span>
        </p>
        <p className="pb-2">
      ...
      <div className="bg-white rounded-xl shadow-lg border-2 border-blue-200 p-5">
  <h2 className="text-base font-semibold mb-5">Prescription Details</h2>
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    filteredAppointments.length > 0 ? (
      ...
      {!showPrescriptionTable && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-00 p-6 ml-5 mr-5 mt-4"
          style={{
                      boxShadow: `
      0 -3px 4px rgba(59, 130, 246, 0.1),   /* Top glow (subtle) */
      -3px 0 4px rgba(59, 130, 246, 0.1),   /* Left glow (subtle) */
      0 3px 4px rgba(59, 130, 246, 0.2),   /* Bottom glow (stronger) */
      3px 0 4px rgba(59, 130, 246, 0.2)    /* Right glow (stronger) */
    `,
                    }}>
            <div className="flex justify-between items-center mb-6">
              <div>
  ...
  >
    ...
    Add Mixed Medicine
            ...
            <div className="overflow-x-auto rounded-xl border border-gray-200 ml-4">
              <table className="w-full border-collapse">
                <thead className="bg-white border-b-2  border-blue-200">
                  <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                    ...
                    <th className="border-b border-gray-200 bg-white px-4 py-4 text-sm font-semibold text-gray-700 text-left">
                      Prescription Type
                    </th>
                    <th className="border-b border-gray-200 px-4 py-4 text-sm font-semibold text-gray-700 text-left">
    ...
    if (item.isMixedMedicine) {
      return (
        <React.Fragment key={item.id}>
          ...
          <tr className="bg-purple-50 hover:bg-purple-100 transition-colors duration-150 border-b border-blue-200">
            <td className="px-4 py-4 text-center" rowSpan={item.mixedMedicines.length + 1}>
              <span className="inline-flex items-center justify-center w-8 h-8 text-purple-800  text-sm font-medium">
                {index + 1}
              </span>
            </td>
            
            {/* Common fields for mixed medicine */}
            <td className="px-4 py-4 min-w-48" colSpan="">
              <select
  ...
  >
    ...
    <option value="SOS Medicine">SOS Medicine</option>
            ...
            <td className="px-4 py-4 min-w-44 " colSpan="1">
              <select
  ...
  >
    ...
    <option value="Parallel + Gap">Parallel + Gap</option>
            ...
            <td colSpan="5" className="px-0 py-4 text-center text-gray-400 ">
              {/* Nested Medicine Rows */}
          {item.mixedMedicines.map((medicine, medIndex) => (
            <tr key={medicine.id} className="bg-white border-l-2 border-purple-300">
              <td className="px-0 py-3 pl-2 w-18" colSpan="2">
                <div className="flex items-center gap-2 space-x-1">
                  ...
                  <select
                    ...
                    className="flex-1 border border-gray-200 rounded-lg px-2 mr-5 py-2 text-sm w-28"
                  >
              ...
              {/* Medicine Form */}
              <td className="px-9 py-0 w-26 bg-gray-100">
                <select
                  value={medicine.form}
                  onChange={(e) => updateMixedMedicine(item.id, medicine.id, "form", e.target.value)}
                  className="w-full border border-gray-200 rounded-lg px-0 py-2 text-sm mr-16 ml-1"
                >
              ...
              {/* Dispense Quantity */}
              <td className=" pl-7 pr-2 px-4 py-4 min-w-32">
                          {item.form === "Liquid form" ? (
                            <select
...
<td className="px-4 py-4 pl-5 bg-gray-100">
  <button
    type="button"
    onClick={() => {
      // Open modal and remember which item + medicine we're editing
      setSelectedModalContext({ itemId: item.id, medicineId: medicine.id });
      setIsRawMaterialModalOpen(true);
    }}
    className="w-full border border-gray-200 rounded-lg px-3 py-2 pl-8 text-sm text-left bg-white hover:bg-gray-50"
  >
    RawMaterials ({medicine.rawMaterials?.length || 0})
  </button>
</td>

{/* ---- Add this modal at the bottom of your component's return (outside the table) ---- */}
{isRawMaterialModalOpen && (
  <div
    className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white bg-opacity-40"
  ...
  >
    ...
    >
      <div className="p-4 border-b flex justify-between items-center">
        <h3 className="font-medium">Select Raw Materials</h3>
        <button
          onClick={() => setIsRawMaterialModalOpen(false)}
        ...
        {filteredRawMaterials.map((material) => (
          <div key={material._id} className="grid grid-cols-2 px-3 py-2 hover:bg-blue-50">
            <label className="flex items-center text-sm cursor-pointer">
              <input
                type="checkbox"
                // 🔥 Keep your original checked logic — but use the medicine from current state
...
<td className="px-4 py-4 min-w-52">
      <div className="space-y-2">
        ...
        {(!medicine.preparationQuantity || medicine.preparationQuantity.length === 0) && (
          <div className="p-3 text-center gap-2 justify-between">
            ...
            {item.mixedMedicines.length > 1 && (
                  <button
                    ...
                    className="inline-flex items-center pl-5 px-2 py-1 bg-red-500 text-white rounded text-xs"
                  >
                    <IoMdTrash />
                  </button>
                )}</span>
...
<td className="px-4 py-4 min-w-32">
  ...
  <div className="flex items-center gap-2">
    ...
    {formToUOM[item.form] && (
      <span className="text-gray-600 text-sm whitespace-nowrap flex-shrink-0">
        {formToUOM[item.form]}
      </span>
    )}
  </div>
...
<td className="px-4 py-4">
  ...
  />
          ...
          <tr className="bg-purple-50">
            <td colSpan="14" className="px-4 py-2 text-right mb-1">
              <button
              ...
              >
                <IoIosAdd /> Add Medicine to Group
                    ...
                    return (
                      <tr
                        key={item.id}
                        className="hover:bg-blue-50 transition-colors duration-150 border-b border-blue-200"
                      >
                        {/* S.No */}
                        <td className="px-4 py-4 text-center bg-gray-100">
                          <span className="inline-flex items-center justify-center w-8 h-8  text-gray-800  text-sm font-medium">
                            {index + 1}
                          </span>
                        </td>
                        ...
                        {/* Medicine Consumption Type */}
                        <td
                          className={`px-4 py-4 min-w-44 bg-gray-100 ${
                        ...
                        {/* Medicine Form */}
                        <td className="px-4 py-4 min-w-36 bg-gray-100">
  <select
    value={item.form}
    onChange={(e) =>
  ...
  >
    ...
    <option value="Individual Medicine">Individual Medicine</option>
                        ...
                        <td
  className={`px-4 py-4 min-w-44 bg-gray-100 ${
    !fieldVisibility.rawMaterial ? "opacity-30 pointer-events-none" : ""
  }`}
>
  <button
    type="button"
  ...
  >
    <span>Raw Materials ({item.rawMaterials.length})</span>
    <svg
      className="w-4 h-4 text-gray-400"
      fill="none"
      stroke="currentColor"
  ...
  >
    ...
    >
      ...
      <div className="p-4 border-b border-gray-200 flex justify-between items-center">
        <h3 className="font-medium text-gray-800">Select Raw Materials</h3>
        <button
          onClick={() => setIsRawMaterialModalOpen(false)}
          className="text-gray-500 hover:text-gray-800"
        >
      ...
      {/* List Header */}
      <div className="grid grid-cols-2 text-sm font-semibold text-gray-600 px-3 py-2 bg-gray-100 border-b">
      ...
      <div className="max-h-80 overflow-y-auto">
        {filteredRawMaterials.length > 0 ? (
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleRawMaterialSelection(
                      currentItemId, // Use modal context
                      material._id,
                      e.target.checked
                    )
                  }
          ...
          <p className="px-3 py-4 text-gray-500 text-sm italic">
      ...
      <div className="p-4 border-t border-gray-100 flex justify-end">
        ...
        >
          Done
                        ...
                        {/* Preparation + Quantity */}
                        <td
                          className={`px-4 py-4 min-w-52 hover:bg-blue-50 ${
                        ...
                        {/* Duration */}
                        <td
                          className={`px-4 py-4 min-w-32 bg-gray-100 hover:bg-blue-50 ${
                            !fieldVisibility.duration
                              ? "opacity-30 pointer-events-none"
                              : ""
                          }`}
                        ...
                        {/* Medicine Consumption */}
                        <td
  className={`px-4 py-4 min-w-64 bg-gray-100 hover:bg-blue-50 ${
    ...
    {formToUOM[item.form] && (
      <span className="text-gray-600 text-sm">
        {formToUOM[item.form]}
                        ...
                        {/* Additional Comments */}
                        <td className="px-4 py-4 min-w-48 bg-gray-100">
                          <textarea
                            value={item.additionalComments}
                            onChange={(e) =>
                              updatePrescriptionItem(
...
<div className=" p-6 bg-white  rounded-xl shadow-lg px-6 h-9px py-2 pb-2 pl-[20px] border border-blue-100 ml-4 mr-4"
          style={{
                      boxShadow: `
      0 -3px 2px rgba(59, 130, 246, 0),   /* Top glow (subtle) */
      -3px 0 2px rgba(59, 130, 246, 0.1),   /* Left glow (subtle) */
      0 3px 4px rgba(59, 130, 246, 0.2),   /* Bottom glow (stronger) */
      3px 0 4px rgba(59, 130, 246, 0.2)    /* Right glow (stronger) */
    `,
                    }}>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
      ...
      />
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16 mt-8 pl-[20px] pr-[20px]">
          {/* Summary Section */}
          <div className=" p-6 bg-white  rounded-xl shadow-lg px-6 h-9px py-2 pb-2 pl-[20px] border border-blue-100 "
          style={{
                      boxShadow: `
      0 -3px 2px rgba(59, 130, 246, 0),   /* Top glow (subtle) */
      -3px 0 2px rgba(59, 130, 246, 0.1),   /* Left glow (subtle) */
    ...
    `,
            ...
            <h2 className="text-lg font-semibold mb-4 mt-5">Prescription Summary</h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 ml-6">
              ...
              <div>
                ...
                <input
                  ...
                  readOnly
                  className="w-full border rounded-xl px-3 py-3 bg-gray-100 font-medium"
                />
              </div>
            </div>
...
style={{
  ...
  <div className="grid grid-cols-1 md:grid-cols-1 gap-4 ml-6">
    ...
    <div>
      ...
      <input
        ...
        readOnly
        className="w-full border rounded-xl px-3 py-3 bg-gray-100 font-bold text-green-800"
      />
    </div>
    <div className="mt-1">
  <label className="block text-base font-medium mb-1">
  ...
  <button
    ...
    className="px-6 py-2 ml-5 border border-gray-300 rounded hover:bg-gray-50"
  >
